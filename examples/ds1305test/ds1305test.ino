 ...
 * File                 ds1305test.ino
 *
 * Synopsis             Test Sketch for DS1305 library
 *
...
/* You should ONLY define __CHARGING_SUPPORTED is you have a chargable battery or super-cap
   properly connected, per DS1305 spec sheet */
...
bool read_write_compare_alarm24(__FlashStringHelper *tc, int alarm, unsigned char hours, unsigned char mins, unsigned char secs, unsigned char dow)
{
  ...
  compare(ts_in.hours, ts_out.hours, "Hours", &pass);
  if (ts_in.hours != DS1305_ANY) compare(hours12, ts_out.hours12, "Hours (12)", &pass);
  if (ts_in.hours != DS1305_ANY) compare(ampm, ts_out.ampm, "AMPM Flag", &pass, true);
  compare(ts_in.minutes, ts_out.minutes, "Minutes", &pass);
  compare(ts_in.seconds, ts_out.seconds, "Seconds", &pass);
  compare(ts_in.dow, ts_out.dow, "DOW", &pass);
...
bool read_write_compare_alarm12(__FlashStringHelper *tc, int alarm, unsigned char hours, char ampm, unsigned char mins, unsigned char secs, unsigned char dow)
{
  ...
  hours24 = convert_to_24(ts_in.hours12, ampm);
  compare(ts_in.hours12, ts_out.hours12, "Hours", &pass);
  if (ts_in.hours12 != DS1305_ANY) compare(ts_in.ampm, ts_out.ampm, "AM PM Flag", &pass);
  if (ts_in.hours12 != DS1305_ANY) compare(hours24, ts_out.hours, "Hours (24)", &pass);
  compare(ts_in.minutes, ts_out.minutes, "Minutes", &pass);
  compare(ts_in.seconds, ts_out.seconds, "Seconds", &pass);
  compare(ts_in.dow, ts_out.dow, "DOW", &pass);
...
int clocktests()
{
  ...
  if (!read_write_compare_clock24(F("RWC24.01"), 0, 1, 1, 0, 0, 0, DS1305_SUNDAY)) failures++;
  
  // RWC24.02 - Upper bound (maximum supportable values)
  if (!read_write_compare_clock24(F("RWC24.02"), 99, 12, 31, 23, 59, 59, DS1305_SATURDAY)) failures++;
  
  // RWC24.03 - Check operation of 12th hour for PM decode, unique values for ALL fields
  if (!read_write_compare_clock24(F("RWC24.03"), 1, 2, 3, 12, 4, 5, DS1305_SATURDAY)) failures++;
  ...
  if (!read_write_compare_clock12(F("RWC12.01"), 0, 1, 1, 0, 'A', 0, 0, DS1305_SUNDAY)) failures++;
  
  // RWC12.02 - Upper bound (maximum supportable values)
  if (!read_write_compare_clock12(F("RWC12.02"), 99, 12, 31, 11, 'P', 59, 59, DS1305_SATURDAY)) failures++;
  
  // RWC12.03 - Check operation of 12th hour for PM decode, unique values for ALL fields
  if (!read_write_compare_clock12(F("RWC12.03"), 1, 2, 3, 9, 'P', 4, 5, DS1305_SATURDAY)) failures++;
...
{  
  ...
  if (!read_write_compare_alarm24(F("ALM24.01"), 0, 0, 0, 0, DS1305_SUNDAY)) failures++;
  
  // ALM24.02 - Upper bound (highest supportable values)
  if (!read_write_compare_alarm24(F("ALM24.02"), 0, 23, 59, 59, DS1305_SATURDAY)) failures++;
  
  // ALM24.03 - Check operation of 12th hour for PM decode, unique values for ALL fields
  if (!read_write_compare_alarm24(F("ALM24.03"), 0, 12, 2, 3, DS1305_FRIDAY)) failures++;

  // Repeat previous 3 tests for second alarm (alarm 1)
  if (!read_write_compare_alarm24(F("ALM24.11"), 1, 0, 0, 0, DS1305_SUNDAY)) failures++;
  if (!read_write_compare_alarm24(F("ALM24.12"), 1, 23, 59, 59, DS1305_SATURDAY)) failures++;
  if (!read_write_compare_alarm24(F("ALM24.13"), 1, 12, 2, 3, DS1305_FRIDAY)) failures++;
  ...
  if (!read_write_compare_alarm24(F("ALM24.21"), 0, DS1305_ANY, 1, 2, DS1305_SUNDAY)) failures++;
  if (!read_write_compare_alarm24(F("ALM24.22"), 1, 1, DS1305_ANY, 2, DS1305_SUNDAY)) failures++;
  if (!read_write_compare_alarm24(F("ALM24.24"), 0, 1, 2, DS1305_ANY, DS1305_SUNDAY)) failures++;
  if (!read_write_compare_alarm24(F("ALM24.23"), 1, 1, 2, 3, DS1305_ANY)) failures++;

  // ALM12 series tests
  // Write alarm (1 or 2), Read back, compare written and read values
  
  // ALM12.01 - Lower bound (lowest supportable values)
  if (!read_write_compare_alarm12(F("ALM12.01"), 0, 12, 'A', 0, 0, DS1305_SUNDAY)) failures++;
  
  // ALM12.02 - Upper bound (highest supportable values)
  if (!read_write_compare_alarm12(F("ALM12.02"), 0, 11, 'P', 59, 59, DS1305_SATURDAY)) failures++;
  
  // ALM12.03 - Check operation of 12th hour for PM decode, unique values for ALL fields
  if (!read_write_compare_alarm12(F("ALM12.03"), 0, 12, 'P', 2, 3, DS1305_FRIDAY)) failures++;

  // Repeat previous 3 tests for second alarm (alarm 1)
  if (!read_write_compare_alarm12(F("ALM12.11"), 1, 0, 'A', 0, 0, DS1305_SUNDAY)) failures++;
  if (!read_write_compare_alarm12(F("ALM12.12"), 1, 11, 'P', 59, 59, DS1305_SATURDAY)) failures++;
  if (!read_write_compare_alarm12(F("ALM12.13"), 1, 12, 'A', 2, 3, DS1305_FRIDAY)) failures++;

  // Do some testing of the ANY indicator in all fields, alternating alarm 1 and alarm 2
  // to ensure that both alarms end up on different values for a further test
  if (!read_write_compare_alarm12(F("ALM12.21"), 0, DS1305_ANY, '\0', 1, 2, DS1305_SUNDAY)) failures++;
  if (!read_write_compare_alarm12(F("ALM12.22"), 1, 1, 'P', DS1305_ANY, 2, DS1305_SUNDAY)) failures++;
  if (!read_write_compare_alarm12(F("ALM12.24"), 0, 1, 'A', 2, DS1305_ANY, DS1305_SUNDAY)) failures++;
  if (!read_write_compare_alarm12(F("ALM12.23"), 1, 1, 'P', 2, 3, DS1305_ANY)) failures++;
...
int runalarmtests2(int alarmnumber, __FlashStringHelper *prefix)
{
  ...
  a.dow = DS1305_ANY;
  a.hours = DS1305_ANY;
  a.minutes = DS1305_ANY;
  a.seconds = DS1305_ANY;
...
// Run tests related to alarm interrupt joining
int alarmtests3()
{
  int failures = 0;
  Serial.print(F("ALMIN.01 Join alarms - "));
  clk24.setAlarmInterruptState(true);
  if (clk24.areAlarmInterruptsJoined()) {
    Serial.println(F("Pass"));
  } else {
    Serial.println(F("Fail"));
    failures++;
  ...
  Serial.print(F("ALMIN.02 Separate alarms - "));
  clk24.setAlarmInterruptState(false);
  if (!clk24.areAlarmInterruptsJoined()) {
    Serial.println(F("Pass"));
  } else {
    Serial.println(F("Fail"));
...
int usermemtests()
{
  ...
  clk24.writeUser(DS1305_USER_START, outbuf, 96);
  clk24.readUser(DS1305_USER_START, inbuf, 96);
...
int crtests()
{
  ...
  Serial.print(F("CROSC.01 Disable Oscillator - "));
  clk24.setRunState(false);
  if (!clk24.isRunning()) {
    Serial.println(F("Pass"));
  } else {
    failures++;
  ...
  Serial.print(F("CROSC.02 Enable Oscillator - "));
  clk24.setRunState(true);
  if (clk24.isRunning()) {
    Serial.println(F("Pass"));
  } else {
    failures++;
